...
//update contentuuid and room id in API
// export const updateRoomId = async (getToken,contentUuid:any): Promise<string> => {
//   const api = 'http://192.168.0.237:8120/4.0.0.1/course/getTeamRoomId';
//   const body : (
//     {
//       "contentUuid" : '',
//       "teamRoomId" : '',
...
//   try {
//     const response = await fetch('url',{nt-Typ
//       method: 'POST',
//       headers: {
//         'Conte'
//       }
...
// };
export const updateRoomId = async (contentUuid: string, teamRoomId: string, mytoken: any): Promise<void> => {
  const api = 'http://192.168.0.237:8120/4.0.0.1/course/getTeamRoomId';
  var getToken = {}
  ...
  try {
    const response = await fetch(`http://192.168.0.237:8120/4.0.0.1/course/getTeamRoomId`, requestOptions);
    if (!response.ok) {
      throw new Error('Unable to Save RoomId');
    }
  } catch (error) {
