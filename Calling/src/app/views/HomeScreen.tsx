...
export const HomeScreen = (props: HomeScreenProps): JSX.Element => {
  ...
  const [presenter, setPresenter] = useState<any>(localStorage.getItem('presenter'));
  const [userName, setUserName] = useState<any>(localStorage.getItem('name'));
  const imageProps = { src: heroSVG.toString() };
  const headerTitle = props.joiningExistingCall ? 'Join Call' : 'Start or join a call';
  const callOptionsGroupLabel = presenter == 1 ? 'Select a call option' : 'Join Call';
  const buttonText = 'Next';
  const callOptions: ICallChoiceGroupOption[] = presenter == 1 ? [{ key: 'StartRooms', text: 'Start a Rooms call' }] : [{ key: 'Rooms', text: 'Join a Rooms Call' }];
  ...
  const roomRoleOptions: IChoiceGroupOption[] = presenter == 1 ?  [{ key: 'Presenter', text: 'Presenter' }] : [{ key: 'Attendee', text: 'Attendee' }];

  // Get display name from local storage if available
  // const defaultDisplayName = localStorageAvailable ? getDisplayNameFromLocalStorage() : null;
  const [displayName, setDisplayName] = useState<string | undefined>(userName ?? undefined);
  ...
  const defaultKeyVal = presenter == 1 ? 'StartRooms' : 'Rooms'
  const buttonEnabled =
          ...
          <Stack tokens={callContainerStackTokens}>
            {!props.joiningExistingCall && (
              <ChoiceGroup
                ...
                defaultSelectedKey= {defaultKeyVal}
                options={callOptions}
                required={true}
                onChange={(_, option) => {
