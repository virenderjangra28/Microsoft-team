...
const App = (): JSX.Element => {
  ...
  const [contentUuid, setContentUuid] = useState<any>();
  ...
  useEffect(()=> {
    const handleMessage = (event) => {
      if(event.origin === 'http://localhost:4200') {
        ...
        setGetToken(event.data.token);
        setContentUuid(event.data.contentuuid);
        setEnvironmentData(event.data.environemnt);
        localStorage.setItem('presenter',event.data.presenter)
        localStorage.setItem('name',event.data.name)
        localStorage.setItem('environemnt',event.data.environemnt)
          ...
          startCallHandler={async (callDetails) => {
            ...
            if (callDetails.option === 'StartRooms') {
              ...
              try {
                ...
                setTimeout(() => {
                  updateRoomId(contentUuid,roomId,getToken)
                }, 100);
              } catch (e) {
                console.log(e);
              }
